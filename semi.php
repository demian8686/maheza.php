...
echo '[ '.date('H:i:s').' ] 1. SMSHUB.ORG / 2. SMS-ACTIVE.ORG / 3. MANUAL OTP / 4. WITH BURP SUITE / HTTP CANARY'.PHP_EOL;
...
echo "\n";
    ...
    for ($ia=0; $ia < $totalReff; $ia++) {
        ...
        echo "\n";
        echo '[ '.date('H:i:s').' ] Kode Reff: '.$codeReff.' Refferal Ke '.$ia.''.PHP_EOL;
        echo "\n";
        ...
        if(preg_match("/ACCESS_NUMBER/", $getNumber)) {
            ...
            if($checkOTPRegist == true) {
                ...
                $time = time();
                echo '[ '.date('H:i:s').' ] Sedang Menunggu OTP';
                CheckUlang:
                $funcOTPRegist = GetOtp($url, $apikey, $idOrder);
                preg_match('#STATUS_OK:(.*)#', $funcOTPRegist, $otp);
                $otp = $otp[1];
                if(empty($otp)){
                    if(time()-$time>60){
                        echo '[ '.date('H:i:s').' ] Gagal Mendapatkan Otp ';
    ...
    if(preg_match("/ACCESS_NUMBER/", $getNumber)) {
        ...
        if($checkOTPRegist == true) {
            ...
            if($errMsg == 'Aktivitas login tidak biasa di perangkat') {
                ...
                if($errMsg == 'Nomor telepon yang Anda masukkan sudah terdaftar.' || $success == true) {
                    echo '[ '.date('H:i:s').' ] Berhasil Mendaftar '.$nomorhp.''.PHP_EOL;
                    ...
                    Login:
                    $inputString = input('Input Parameter Data : ');
                    ...
                    $data = $inputString;
                    $login = curl('https://app.oneaset.co.id/api/app/user/login/v2', $data, $headers);
                    if (strpos($login[1], '"accessToken":"')) {
                        echo '[ '.date('H:i:s').' ] Berhasil Login Refferal Reward'.PHP_EOL;
                    } else {
                        echo '[ '.date('H:i:s').' ] Failure Login'.PHP_EOL;
                        goto Login;
                    }
                    echo "\n";
